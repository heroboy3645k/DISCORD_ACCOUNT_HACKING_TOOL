...
class User:
    ...
    def get_server(self, server_id):
        self.current_path = f"guilds/{server_id}/channels"
        header = self.make_get_header(self.current_path)
        ...
        self.guild_in = server_id

        self.get.refresh_ua()

    def get_channel(self, channel_id, num=10):

        self.channel_in = channel_id
    ...
    def get_server_users(self, server_id):
        self.current_path = f"guilds/{server_id}/members"
        self.guild_in = server_id
    ...
    def search(self, _id):
        ...
        self.current_path = f'users/{_id}/'
        header = self.make_get_header(self.current_path)

        res = requests.get(API_VERSION + self.current_path, headers=header,
                           params={"with_mutual_guilds": "true", "guild_id": self.guild_in}).text

        res = json.loads(res)

        print(res)

    def create_dm(self, _id):

        self.current_path = '/users/@me/channels'

        header = self.make_get_header(self.current_path)

        response = requests.post(API_VERSION + self.current_path, headers=header, json={"recipients": [_id]})
